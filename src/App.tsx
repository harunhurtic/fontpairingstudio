...
export default function App() {
  ...
  const handleStyleContrastChange = (contrast: string) => {
    ...
    if (contrast !== "custom") {
      ...
      if (headerFontData && bodyFontData) {
        const headerWeights = headerFontData.weights;
        const bodyWeights = bodyFontData.weights;

        // Check if fonts are same category (serif/sans-serif)
        const headerIsSerif =
          headerFontData.category === "serif";
        const bodyIsSerif = bodyFontData.category === "serif";
        const sameCategory = headerIsSerif === bodyIsSerif;

        switch (contrast) {
          case "low":
            // Low contrast: similar weights, same styles, prefer same categories
            if (sameCategory) {
              // Same category - use very similar weights
              const midWeight =
                headerWeights[
                  Math.floor(headerWeights.length / 2)
                ];
              setHeaderWeight(
                headerWeights.includes(500) ? 500 : midWeight,
              );
              setBodyWeight(
                bodyWeights.includes(400)
                  ? 400
                  : bodyWeights.find(
                      (w) => Math.abs(w - 450) < 100,
                    ) || bodyWeights[0],
              );
            } else {
              // Different categories - try to find fonts in same category
              const sameAsHeader = googleFonts.filter(
                (f) =>
                  getFontData(f.name)?.category ===
                  headerFontData.category,
              );
              const sameAsBody = googleFonts.filter(
                (f) =>
                  getFontData(f.name)?.category ===
                  bodyFontData.category,
              );

              if (sameAsHeader.length > 1 && !isBodyLocked) {
                const newBodyFont = sameAsHeader.find(
                  (f) => f.name !== currentPair.header,
                );
                if (newBodyFont) {
                  setCurrentPair((prev) => ({
                    ...prev,
                    body: newBodyFont.name,
                  }));
                  const newBodyData = getFontData(
                    newBodyFont.name,
                  );
                  if (newBodyData) {
                    setBodyWeight(
                      newBodyData.weights.includes(400)
                        ? 400
                        : newBodyData.weights[0],
                    );
                  }
                }
              }
          ...
          case "medium":
            // Medium contrast: moderate weight difference, normal styles
            setHeaderWeight(
              headerWeights.includes(600)
                ? 600
                : headerWeights[
                    Math.floor(headerWeights.length * 0.7)
                  ],
            );
            setBodyWeight(
              bodyWeights.includes(400) ? 400 : bodyWeights[0],
            );
            setHeaderStyle("normal");
            setBodyStyle("normal");
            break;
          ...
          case "high":
            // High contrast: maximum weight difference, consider different categories
            setHeaderWeight(
              headerWeights[headerWeights.length - 1],
            ); // Boldest
            setBodyWeight(bodyWeights[0]); // Lightest

            // For high contrast, prefer different font categories if possible
            if (
              sameCategory &&
              !isBodyLocked &&
              !isHeaderLocked
            ) {
              const differentCategory = googleFonts.filter(
                (f) => {
                  const fontData = getFontData(f.name);
                  return (
                    fontData &&
                    fontData.category !==
                      headerFontData.category
              ...
              if (differentCategory.length > 0) {
                const newBodyFont =
                  differentCategory[
                    Math.floor(
                      Math.random() * differentCategory.length,
                    )
                  ];
                setCurrentPair((prev) => ({
                  ...prev,
                ...
                if (newBodyData) {
                  setBodyWeight(newBodyData.weights[0]);
                }
              }
            }
